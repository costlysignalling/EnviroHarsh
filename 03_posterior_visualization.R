...
png("posterior_visual.png",width=(w1/rat[1])*sum(rat),height=28,units="cm",res=600)
...
(prec<-precis(m.big,depth=2,omit=c("zf","zr","zfA","zrA","ziA","zfF","zrF","ziF","zfH","zrH","ziH")))

#convert for visualizetion in Corel

clr<-colorRampPalette(c("#800000","#FF0000","#E0E0E0","#00FF00","#008000"))

gradient<-clr(2001)
num<-seq(-1,1,l=2001)

nampar<-rownames(prec)
colline<-sapply(prec$mean,function(x){gradient[which.min(abs(num-x))]})
size1<-round(abs(prec$mean)*10,2)
size2<-round(abs(prec$mean)*20,2)
size3<-round(abs(prec$mean)*50,2)
size4<-round(abs(prec$mean)*80,2)

visdat<-data.frame(nampar,colline,size1,size2,size3,size4)

write.table(visdat,"visdat.txt",sep="\t",row.names = F)
